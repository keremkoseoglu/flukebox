...
_PLAYLIST_WINDOW = webview.create_window(
    "FlukeBox", _APP, width=896, height=1120, x=0, y=0, text_select=True
)
_PLAYER_WINDOW = webview.create_window(
    "Player", html="<body bgcolor=black />", width=896, height=1120, x=896, y=0
)
_START_PLAYLIST = ""
...
def start_playlist():
    """Main view"""
    output = {"start_playlist": _START_PLAYLIST, "no_local": _START_NO_LOCAL}
...
def api_generate():
    ...
    if len(_SEEKER_SONGS) > 0:
        ...
        songs = _PRODUCER.build_song_list(
            request.args.get("playlist"), no_local=no_local, purify=True
        )
...
def start_gui(
    ...
    threading.Thread(
        target=lambda: _APP_EXTERNAL.run(host="0.0.0.0", port=ext_port)
    ).start()
    webview.start()
